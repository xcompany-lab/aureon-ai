 ...
 *
 * Usage:
 *   npx aureon-ai install [name] - Install Aureon AI (optional project name)
 *   npx aureon-ai validate   - Validate MoneyClub email
 *   npx aureon-ai push       - Push to Layer 1/2/3 remote
 *   npx aureon-ai upgrade    - Upgrade Community to Premium
 *   npx aureon-ai status     - Show Pro license status
 *   npx aureon-ai features   - List available vs locked features
 *   npx aureon-ai --help     - Show help
 */
...
async function main() {
  ...
  if (!skipEnvCheck) {
    ...
    if (!existsSync(projectEnv)) {
      ...
      console.log(boxen(
        ...
        '  (You can run this anytime with: npx aureon-ai setup)',
  ...
  if (command === 'install') {
    ...
    if (typeof runUpgrade === 'function') {
      ...
      console.log('  Por enquanto, reinstale com: aureon install\n');
    }
...
function showHelp() {
  console.log(`
  Aureon AI v${pkg.version}
  AI Knowledge Management System

  Comandos:
    install [nome]  Instalar Aureon AI (PREMIUM ou Community)
    setup       Configurar API keys e dependencias (wizard interativo)
    validate    Validar email MoneyClub (aureon validate <email>)
    push        Push para Layer 1/2/3 (aureon push [--layer N])
  ...
  Exemplos:
    npx aureon-ai install
    npx aureon-ai install meu-projeto
    npx aureon-ai push --layer 1
    npx aureon-ai push
`);
}
